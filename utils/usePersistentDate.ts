import { date1GreaterThanDate2 } from "@/utils/dateUtils";
...
export const usePersistentDate = (dateSinceRestart: Date | undefined) => {
  ...
  useEffect(() => {
    if (
      dateSinceRestart &&
      date1GreaterThanDate2(dateSinceRestart, localDate)
    ) {
...
const fetchLastUpdateCheckDateAsync: () => Promise<
  Date | undefined
> = async () => {
...
const storeLastUpdateCheckDateAsync: (date: Date) => Promise<void> = async (
  date,
) => {
